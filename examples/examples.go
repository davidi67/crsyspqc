...
//
//	fmt.Println(headtail("abcdefghijklmnopqrstuvwxyz", 5))
//	// abcde
//	// ...vwxyz
...
func main() {
	...
	fmt.Println("\nTesting SLHDSA-DSA-PreHash...")
	// Use SLHDSA-DSA-PreHash.Sign and .Verify
	// Pass the value of "ph" computed externally and the hash algorithm id
	testmap = T_slhdsa_prehash_1 // In datatests.go
	// Get test values from testmap
	alg, err = dsa.LookupAlgByName(testmap["alg"])
	if err != nil {
		panic(err)
	...
	fmt.Println("Alg:", dsa.AlgName(alg))
	hashalg, err := lookupHashAlg(testmap["hashAlg"])
	if err != nil {
		panic(err)
	}
	fmt.Println("hashAlg:", testmap["hashAlg"])
	ph, _ := hex.DecodeString(testmap["ph"])
	sk, _ = hex.DecodeString(testmap["sk"])
	ctx, _ = hex.DecodeString(testmap["context"])
	rndhex = testmap["rnd"]
	fmt.Println("ph:", hex.EncodeToString(ph))
	sig, _ = dsa.SignPreHash(alg, ph, hashalg, sk, 0, ctx, rndhex)
	fmt.Println("sig:", headtail(hex.EncodeToString(sig), 96))
	// Check this matches the known result
	if b, _ := hex.DecodeString(testmap["signature"]); !slices.Equal(b, sig) {
		fmt.Println("SIGNATURE IS WRONG")
	...
	// Get the public key from the private key
	pk, _ = dsa.PublicKeyFromPrivate(alg, sk)
	// Then verify the signature using pre-hash version
	ok, err = dsa.VerifyPreHash(alg, sig, ph, hashalg, pk, 0, ctx)
	if err != nil {
		fmt.Println(err)
		return
	}
	fmt.Println("Dsa.VerifyPreHash returns", ok)
	...
	ss, ct, _ = kem.EncapsWithParams(algk, ek, rndhex)
	fmt.Println("ct:", headtail(hex.EncodeToString(ct), 64))
	fmt.Println("ss: ", headtail(hex.EncodeToString(ss), 64))
	// Check this matches the known result
